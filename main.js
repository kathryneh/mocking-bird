...
var Facebook_Extension = {
    ...
    show_highlight_btn : function(){
        ...
        mockbrd_btn.css({
	    	...
	    	{
			if(self.highlighted_text.length <= 260)
    ...
    getKeywords : function(text){
    	var self = this;
	var keywords;
	var link = 'http://mocking-bird.herokuapp.com/curl.php';
	$.ajax({
		url : link,
		type: 'GET',
		data: {
		...
		success: function(data, textStatus, jqXHR) {
		if(data.status[0].code==0){
			keywords = data.text[0];
			window.content.alert(data.text[0]);
		}
		else
			keywords = text;
	...
	error : function(jqXHR, textStatus, errorThrown) {
		window.content.alert("Shit went down.");
	}
	});
    },
    ...
    facebook_app_init : function(){
        var self = this;
        if(this.page_content.find('#fb_root').size()){
            this.page_content.find('body').append('<div id="fb_root"></div>');
        }
        ...
        $('.mockbtn'+this.randomClass).unbind('click').click(function(){
            FB.ui({
                    method: 'feed',
                    message: 'I just found this song!',
                    link: self.page_content.find('.hidden-video-player'+self.classRandom).attr('src')
            })
        });
        window.content.alert('fb_appended');
    },
